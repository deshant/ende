...
public class V_auto_act extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_v_auto_act);
		...
		String kvalue_temp = intent.getStringExtra(Vigenere.KEY);
		String kvalue = kvalue_temp.replaceAll("[^\\w\\s]", "");
		String msg = message.replaceAll("[^\\w\\s]", "");

		String autoKey = kvalue + msg; // for encryption
		System.out.println(autoKey);
		byte[] key_text = null;
		byte[] autoKey_text = null;

		// converting for easy manipulation of characters
		try {
			plain_text = msg.getBytes("US-ASCII");
		...
		try {
			...
			autoKey_text = autoKey.getBytes("US-ASCII");

		} catch (UnsupportedEncodingException e1) {
			e1.printStackTrace();
		}

		byte[] autoKey_gen = new byte[autoKey_text.length];
		autoKey_gen = autoKey_text;
		...
		if (act.equals("encrypt")) {
			for (int i = 0, j = 0; i < plain_text.length; i++) {
				...
				if (j >= autoKey_text.length)
					j = 0;

				int value = (int) plain_text[i];
				int mod = (int) autoKey_text[j];
			...
			for (int i = 0, j = 0; i < plain_text.length; i++) {
				...
				if (value >= 65 && value <= 90) {
					...
					if (value - mod > 64) {
						value = value - mod;
						plain_text[i] = (byte) value;
						autoKey_gen[key_text.length + i] = plain_text[i];
					} else {
						mod = 65 - (value - mod);
		...
		Button btn = (Button) getLayoutInflater().inflate(
				R.layout.template_btn, null);
		btn.setText("COPY");
		btn.setTextSize(18);
		btn.setId(3);
